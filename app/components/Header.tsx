...
export default function Header() {
  ...
  return (
    <header
      className={`fixed z-50 w-full bg-zinc-400/80  ${inter.className} antialiased`}
    >
      <div className="mx-auto flex max-w-sm items-center justify-between px-4 py-2">
        {/* Логотип */}
        <Image
          src="/logo.png"
          alt="logo Icon"
        ...
        <motion.div
          whileTap={{ scale: 0.95 }}
          className="relative flex flex-col rounded-lg bg-zinc-900/0 p-2"
        >
          <h1 className="text-[1.2rem] leading-4.5 font-extrabold text-[#2E5235] ">
        ...
        >
          <div
            className={`relative top-2 flex text-gray-950 transition-all duration-300 dark:text-gray-200 ${
              isOpen
                ? "translate-y-5 scale-0 opacity-0"
        ...
        <nav className="relative hidden max-w-full font-normal uppercase md:flex md:items-center md:justify-center">
          <ul className="flex space-x-12">
            ...
            <li className="overflow-hidden">
              <Link
                href="/"
                className={`" relative inline-block py-[0.1em] text-gray-700 transition-all duration-300  ${pathname.startsWith("/projects") ? "translate-y-full" : " "}`}
              >
                Projects
              </Link>
            </li>
